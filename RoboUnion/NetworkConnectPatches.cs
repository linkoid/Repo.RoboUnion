...
internal static class NetworkConnectPatches
{
    ...
    static void Patch()
    {
        var method_OnCreateRoomFailed = AccessTools.Method(typeof(NetworkConnect), nameof(NetworkConnect.OnCreateRoomFailed));
        var method_OnJoinRoomFailed = AccessTools.Method(typeof(NetworkConnect), nameof(NetworkConnect.OnJoinRoomFailed));
        var method_TryJoiningRoom = AccessTools.Method(typeof(NetworkConnect), nameof(NetworkConnect.TryJoiningRoom));

        if (RoboUnion.UseV04MaxPlayersLogic) return; // None of these patches are necessary in v0.4.0

        HarmonyMethod onCreateRoomFailedPrefix;
        if (method_OnCreateRoomFailed.GetParameters().Any(x => x.Name == "cause"))
        {
            onCreateRoomFailedPrefix = new HarmonyMethod(((Delegate)OnCreateRoomFailed_Prefix_v0_1_2_22).Method);
        }
        else
        {
            onCreateRoomFailedPrefix = new HarmonyMethod(((Delegate)OnCreateRoomFailed_Prefix).Method);
        }

        RoboUnion.Instance.Harmony!.Patch(method_OnCreateRoomFailed, prefix: onCreateRoomFailedPrefix);
        RoboUnion.Instance.Harmony!.Patch(method_OnJoinRoomFailed, prefix: onCreateRoomFailedPrefix);

        HarmonyMethod tryJoiningRoomTranspilier = new HarmonyMethod(((Delegate)TryJoiningRoom_Transpiler).Method);
        RoboUnion.Instance.Harmony!.Patch(method_TryJoiningRoom, transpiler: tryJoiningRoomTranspilier);
    }

    [HarmonyPostfix]
    [HarmonyPatch(nameof(NetworkConnect.OnCreatedRoom))]
    static void OnCreatedRoom_Postfix()
    {
        RoboUnion.Logger.LogInfo($"Photon room created. MaxPlayers = {PhotonNetwork.CurrentRoom.MaxPlayers}");
    }
